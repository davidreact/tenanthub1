...
-- This ensures that INSERT operations are allowed when the conditions are met
...
-- Property manager policies for other tables (simplified)
...
USING ((auth.jwt() -> 'user_metadata' ->> 'role') = 'property_manager')
WITH CHECK ((auth.jwt() -> 'user_metadata' ->> 'role') = 'property_manager');
